...
def generate_mock_timeseries():
    ...
    lake_configs = {
        ...
        "sankey": {"base_before": 24.0, "base_after": 24.0, "event_date": None, "std_dev": 1.5},
        "ulsoor": {"base_before": 58.0, "base_after": 58.0, "event_date": None, "std_dev": 2.0},
    }

    for lake in collection.lakes:
        cfg = lake_configs.get(
            lake.id, {"base_before": 50.0, "base_after": 50.0, "event_date": None, "std_dev": 2.0}
        )
        ...
        for start, end in windows:
            ...
            if cfg["event_date"] and start >= cfg["event_date"]:
                ...
                months_since = (start.year - cfg["event_date"].year) * 12 + (
                    start.month - cfg["event_date"].month
                )
                if months_since < 6:

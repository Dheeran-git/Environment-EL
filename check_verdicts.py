...
events = json.loads(pathlib.Path('src/bangalore_lakes/data/restoration/restoration_events.json').read_text())['events']
...
for ev in events:
    ...
    scores = [(date.fromisoformat(r['month_start']), r['pollution_score']) for r in data if r['pixel_count'] > 0]
    ...
    if pre and post:
        ...
        print(f"  pre_avg={pre_avg:.2f}  post_avg={post_avg:.2f}  diff={diff:+.2f}  verdict={label}")

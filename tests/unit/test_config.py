...
def test_defaults_without_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)  # avoid picking up a .env from repo
    s = get_settings()
...
def test_env_vars_are_prefixed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_log_level_validated(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
    with pytest.raises(ValueError, match="log_level"):
        Settings(log_level="LOUD")


def test_log_level_uppercased(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
    s = Settings(log_level="debug")
    assert s.log_level == "DEBUG"


def test_crs_validated(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
    with pytest.raises(ValueError, match="EPSG"):
        Settings(default_crs="wgs84")


def test_bangalore_bbox(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
...
def test_default_days_range(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
